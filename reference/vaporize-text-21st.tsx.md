# Vaporize Text Cycle — интро-экран (отложено)

Источник: 21st.dev (компонент VaporizeTextCycle, React + canvas).
План: показывать при ПЕРВОМ заходе на сайт как интро — текст испаряется в частицы
(например: "SILKWAY" → "Сайты. Боты. Магазины." → уход в hero).

Наш сайт на vanilla JS — компонент нужно ПОРТИРОВАТЬ (это несложно: вся логика
на canvas 2D, React там только обвязка):
- createParticles(): сэмплирование пикселей текста с canvas → массив частиц
- updateParticles(): волна испарения слева направо, физика разлёта, затухание
- renderParticles(): fillRect 1×1 по частицам с alpha
- Состояния: static → vaporizing → fadingIn → waiting → цикл
- После интро: fade-out оверлея, запоминать в localStorage («уже видел») и
  больше не показывать; уважать prefers-reduced-motion (сразу скип).

Параметры из оригинала: spread 5, density 5, vaporizeDuration 2s,
fadeIn 1s, wait 0.5s, direction left-to-right.

Цвета для нас: фон night #0E1F42, текст sand #F7F4EE или gold #C9A227.
Шрифт: Unbounded 600.

---

Оригинальный код (React/TSX) сохранён ниже без изменений.
